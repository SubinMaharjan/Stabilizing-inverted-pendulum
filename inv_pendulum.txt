...
MAX_SEASONS = 50      # total number of training seasons
TRAIN_EPISODES = 10     # total number of episodes in each season
...
class Actor:
    def __init__(self, state_size, action_size,
        ...
        # additions
        # logstd = 4.8481163646 * tf.Variable(np.ones(shape=self.action_size, dtype=np.float32))
...
def validate(env, agent, ep_max=50):
    ...
    for ep in range(ep_max):
        ...
        while True:
            #env.render()
            action = agent.policy(state)*127.5
            next_state, reward, done, info = env.step(action)
            ep_reward += reward
            t += 1
            state = next_state
...
if __name__ == '__main__':
    ...
    # agent.load_model('./', actorfile='actor_weights.h5', criticfile= 'critic_weights.h5')
